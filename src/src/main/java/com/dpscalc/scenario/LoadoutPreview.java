...
public final class LoadoutPreview extends JPanel {
    ...
    public void showLoadout(Scenario.Loadout loadout,OwnedEquipment owned,String profile){
        for(int i=0;i<BankOptimizer.SLOTS.length;i++){
            ...
            if(id>0){
                String ownership=!owned.ready(profile)?"Unknown":owned.owns(id)?"Owned":"Missing";
                label.setText(ownership);label.setBorder(BorderFactory.createLineBorder(!owned.ready(profile)?CalculatorWorkspace.EDGE:owned.owns(id)?CalculatorWorkspace.BEST:new Color(218,126,106)));
                label.setToolTipText(description+" — "+ownership.toLowerCase(Locale.ROOT)+(owned.ready(profile)?" ("+owned.quantities.getOrDefault(id,0)+")":"; open this character's bank"));
                ...
                else {label.setIcon(CalculatorWorkspace.icon("slots/"+ICONS[i],16));label.setText("<html><center>"+CalculatorWorkspace.escape(name.substring(0,Math.min(8,name.length())))+"<br>"+ownership+"</center></html>");}
            }
        }
        repaint();
    }
